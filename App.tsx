    ...
    <div className="min-h-screen pb-20 selection:bg-violet-500 selection:text-white font-inter bg-[#020617] text-slate-100 animate-fade-in flex flex-col relative overflow-hidden">
      ...
      */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => { setResult(null); }}>
             <div className="relative">
                <div className="absolute inset-0 bg-violet-500 blur-lg opacity-20 group-hover:opacity-40 transition-opacity"></div>
                <Dna className="w-8 h-8 text-violet-400 relative z-10 group-hover:rotate-180 transition-transform duration-700" />
             </div>
             <span className="font-brand font-bold text-2xl tracking-tight text-white group-hover:text-violet-200 transition-colors">
                ABK Genomics
             </span>
          </div>
          ...
          <div className="flex items-center gap-4">
             {!result && (
                <div className="hidden md:flex items-center gap-2">
                   ...
                   <span className="text-xs font-mono text-emerald-400 tracking-wider">SYSTEM ACTIVE</span>
                </div>
             )}
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full relative z-10">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-8">
                    <div className="max-w-2xl space-y-6">
                        ...
                        <p className="text-slate-400 text-lg max-w-lg leading-relaxed">
                            Analyze variants using <strong className="text-slate-200">Real-Time Data</strong> from MyVariant.info, ClinVar, and dbNSFP.
                        </p>
                    </div>
                </div>

                {/* Input Card */}
                <div className="glass-panel p-1 rounded-2xl shadow-2xl shadow-violet-900/10">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            <div className="lg:col-span-7 flex flex-col">
                                ...
                                <div className="flex p-1 bg-slate-900 rounded-lg mb-6 w-full sm:w-fit border border-slate-800">
                                    ...
                                    >
                                        <FileJson className="w-3 h-3" /> Example Library
                                    </button>
                                </div>
                                ...
                                <div className="flex-grow bg-slate-950/50 rounded-xl border border-slate-800 relative overflow-hidden min-h-[300px]">
                                    ...
                                    {inputType === 'upload' && (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 group transition-all">
                                            <div className="w-16 h-16 bg-slate-900 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform border border-slate-700 group-hover:border-violet-500/50">
                                                <Upload className="w-6 h-6 text-slate-400 group-hover:text-violet-400 transition-colors" />
                                            </div>
                                            <h4 className="text-white font-bold mb-2">Drag & Drop Genomic File</h4>
                                            <p className="text-slate-500 text-xs mb-6 max-w-xs">Supports .VCF, .TXT (23andMe), .FASTA, .CSV up to 50MB</p>
                                            <label className="px-6 py-2.5 bg-white text-slate-900 rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer hover:bg-violet-50 transition-colors shadow-lg hover:shadow-violet-500/20">
                                                Browse Files
                                        ...
                                        <div className="p-4 grid grid-cols-1 gap-3 overflow-y-auto h-full">
                                            {EXAMPLES.map(ex => (
                                                <button 
                                                    ...
                                                    className="flex items-start gap-4 p-4 rounded-lg bg-slate-900 border border-slate-800 hover:border-violet-500/50 hover:bg-slate-800 transition-all text-left group"
                                                >
                                                    <div className={`mt-1 p-2 rounded bg-slate-950 border border-slate-800 ${inputData === ex.data ? 'text-emerald-400 border-emerald-500/30' : 'text-slate-500 group-hover:text-violet-400'}`}>
                                ...
                                <div className="mt-2 flex justify-between items-center px-1">
                                    <span className="text-[10px] text-slate-600 font-mono">
                                        {inputData ? `${inputData.length} chars loaded` : 'No data loaded'}
                                    </span>
                                    <span className="text-[10px] text-slate-600 uppercase tracking-wider font-semibold">GRCh38 Reference</span>
                                </div>
                            </div>
        ...
        {/* === REAL LOADING: PROCESS STATE === */}
        {loading && (
            <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fade-in-up">
                <div className="glass-panel p-1 rounded-2xl shadow-2xl shadow-violet-900/20 max-w-lg w-full">
                    <div className="bg-[#0f172a]/90 rounded-xl p-8 border border-white/5 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 mb-6 pb-6 border-b border-white/5">
                            <div className="relative">
                                ...
                                <Server className="w-8 h-8 text-violet-400 relative z-10" />
                            </div>
                            <div>
                        ...
                        <div className="flex flex-col items-center justify-center py-8">
                             <div className="w-16 h-16 rounded-full border-4 border-violet-500/30 border-t-violet-500 animate-spin mb-6"></div>
                        ...
                        {/* Scanning Line Animation at bottom */}
                        <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-violet-500 to-transparent opacity-50 animate-[scan_2s_linear_infinite]"></div>
                    </div>
                </div>
            </div>
        )}
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-white/5 pb-6">
                    <div>
                        ...
                        <div className="flex items-center gap-3 text-slate-400 text-sm">
                            <span className="font-mono text-xs px-2 py-0.5 bg-slate-800 rounded text-violet-300 border border-slate-700">ID: {Math.random().toString(36).substr(2, 9).toUpperCase()}</span>
                    ...
                    <div className="flex items-center gap-4">
                        <button 
                            ...
                            className="hidden md:flex items-center gap-2 px-4 py-2 bg-white text-slate-900 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-violet-50 hover:shadow-[0_0_15px_rgba(255,255,255,0.4)] transition-all"
                        >
                ...
                <div className="min-h-[500px]">
                    ...
                    {activeTab === 'overview' && (
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                                {/* Summary Card */}
                                <div className="md:col-span-8 glass-panel p-8 rounded-xl relative overflow-hidden flex flex-col justify-center border-l-4 border-l-violet-500">
                                ...
                                <div className="md:col-span-4 bg-[#050505] rounded-xl border border-white/10 relative overflow-hidden min-h-[250px] group shadow-2xl">
                                     ...
                                     <div className="absolute inset-0 flex items-center justify-center">
                                         {/* Abstract DNA/Twin Animation */}
                                         <div className="relative w-full h-full flex items-center justify-center">
                                              <div className="w-32 h-64 border-x border-violet-500/20 rounded-[100%] absolute animate-pulse"></div>
                                              <div className="w-48 h-48 bg-violet-600/10 blur-3xl rounded-full absolute"></div>
                                              <Fingerprint className="w-20 h-20 text-violet-400 opacity-80" />
                                              {/* Scanning Line */}
                    ...
                    {activeTab === 'variants' && (
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                             {(!result.variants || result.variants.length === 0) && <p className="text-slate-500 col-span-2 text-center py-20 italic">No significant variants detected based on current filter.</p>}
                        ...
                        <div className="space-y-6">
                            <div className="glass-panel p-6 rounded-xl border-l-4 border-l-pink-500">
                                ...
                                <p className="text-slate-400 text-sm">
                                    Based on your genetic profile, these are the predicted phenotypic characteristics, nutritional needs, and fitness traits associated with your "Digital Twin".
                                </p>
                            </div>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                {(!result.phenotypeTraits || result.phenotypeTraits.length === 0) && <p className="text-slate-500 col-span-4 text-center py-20 italic">No phenotype traits detected in this dataset.</p>}
      ...
      <footer className="w-full bg-[#020617]/90 border-t border-white/5 py-4 relative z-10">
         <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-slate-500">
            <div className="flex items-center gap-2">
               ...
               <p>
                  <strong className="text-slate-400">Validated Sources:</strong> Findings are cross-referenced with <span className="text-slate-300">ClinVar</span>, <span className="text-slate-300">PharmGKB</span>, <span className="text-slate-300">dbSNP</span>, and <span className="text-slate-300">CPIC Guidelines</span>.
               </p>
            </div>
